...
class _SubjectResult {
  ...
  Map<String, dynamic> get latestExam => exams.isNotEmpty ? exams.first : {};

  int    get marks        => latestExam['marks_obtained'] as int?  ?? 0;
  String get examType     => latestExam['exam_type']     as String? ?? 'Unit Test';
...
class _StudentMarksScreenState extends State<StudentMarksScreen>
  ...
  void initState() {
    ...
    _listCtrl   = AnimationController(vsync: this, duration: const Duration(milliseconds: 1400));
    _scoreCtrl  = AnimationController(vsync: this, duration: const Duration(milliseconds: 1600));
  ...
  Future<void> _fetchMarks() async {
    ...
    try {
      ...
      if (res['success'] == true && mounted) {
        final List rawSubjects = res['subjects'] ?? [];
        final summary          = res['summary']  as Map<String, dynamic>?;

        setState(() {
          _results = List.generate(rawSubjects.length, (i) {
            final s    = rawSubjects[i] as Map<String, dynamic>;
            final exams = (s['exams'] as List? ?? [])
                .map((e) => Map<String, dynamic>.from(e as Map))
                .toList();
            return _SubjectResult(
              subjectId:  s['subject_id']    as int?    ?? 0,
              name:       s['subject_name']  as String? ?? '',
              icon:       s['subject_icon']  as String? ?? '📚',
              color:      _colorFromHex(s['subject_color'] as String?, i),
          ...
          if (summary != null) {
            _totalObtained = summary['total_obtained']    as int?    ?? 0;
            _totalPossible = summary['total_possible']    as int?    ?? 0;
            _overallPct    = (summary['overall_percentage'] as num?)?.toDouble() ?? 0;
            _overallGrade  = summary['overall_grade']     as String? ?? '';
            _rank          = summary['rank']              as int?;
            _totalInClass  = summary['total_in_class']    as int?;
          }

          _isLoading = false;
        });

        Future.delayed(const Duration(milliseconds: 200), () { if (mounted) _scoreCtrl.forward(); });
        Future.delayed(const Duration(milliseconds: 400), () { if (mounted) _listCtrl.forward();  });
      } else {
              ...
              children: [
                ShaderMask(
                  shaderCallback: (b) => const LinearGradient(
                      colors: [Color(0xFF00D4AA), Color(0xFFFFB347)]).createShader(b),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: const Row(children: [
                ...
                SizedBox(width: 5),
                Text('Refresh', style: TextStyle(color: Color(0xFF00D4AA),
  ...
  Widget _buildOverallScoreCard() {
    return AnimatedBuilder(
      ...
      builder: (ctx, _) {
        final displayPct   = (_overallPct  * _scoreAnim.value).toStringAsFixed(1);
        final displayMarks = (_totalObtained * _scoreAnim.value).toInt();
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: LinearGradient(
              colors: [_overallGradeColor.withOpacity(0.2), const Color(0xFF131929)],
              begin: Alignment.topLeft, end: Alignment.bottomRight,
            ),
            border: Border.all(color: _overallGradeColor.withOpacity(0.3)),
            boxShadow: [BoxShadow(color: _overallGradeColor.withOpacity(0.15),
          ...
          child: Column(
            children: [
              Container(
                height: 5,
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [_overallGradeColor,
              ...
              Padding(
                ...
                child: Column(
                  children: [
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: CircularProgressIndicator(
                                value: _overallPct / 100 * _scoreAnim.value,
                                strokeWidth: 7,
                                backgroundColor: Colors.white.withOpacity(0.06),
                                valueColor: AlwaysStoppedAnimation<Color>(_overallGradeColor),
                                strokeCap: StrokeCap.round,
                              ),
                            ),
                            Column(children: [
                              Text(_overallGrade,
                                  style: TextStyle(color: _overallGradeColor,
                            ...
                            children: [
                              ...
                              RichText(
                                text: TextSpan(children: [
                                  TextSpan(text: '$displayMarks',
                                      style: TextStyle(color: _overallGradeColor,
                                          fontSize: 38, fontWeight: FontWeight.w900, letterSpacing: -2)),
                                  TextSpan(text: ' / $_totalPossible',
                                      style: TextStyle(color: Colors.white.withOpacity(0.3),
                                          fontSize: 18, fontWeight: FontWeight.w500)),
                                ]),
                              ),
                              const SizedBox(height: 6),
                              if (_rank != null && _totalInClass != null)
                                Container(
                                  ...
                                  child: Text('🏆 Rank $_rank / $_totalInClass',
                                      style: TextStyle(color: _overallGradeColor,
                                          fontSize: 12, fontWeight: FontWeight.w700)),
                                )
                              else
                                Container(
                                  ...
                                  child: Text(
                                    _overallPct >= 80 ? '🌟 Excellent Work!' :
                                    _overallPct >= 60 ? '👍 Keep it up!'     : '💪 You can do better!',
                                    style: TextStyle(color: _overallGradeColor,
                                        fontSize: 12, fontWeight: FontWeight.w700),
                                  ),
                                ),
                    ...
                    const SizedBox(height: 20),
                    Row(children: [
                      _miniStat('${_results.length}', 'Subjects',   const Color(0xFF6C63FF)),
                      _miniDivider(),
                      _miniStat('${_results.where((r) => r.exams.length > 1).length}',
                          'Multi Exam', const Color(0xFF00D4AA)),
                      _miniDivider(),
                      _miniStat(
                          '${_results.where((r) => r.grade == 'A+' || r.grade == 'A').length}',
                          'A Grades', const Color(0xFFFF8C42)),
                    ]),
                  ],
                ),
              ),
          ...
          return AnimatedBuilder(
            ...
            builder: (ctx, child) {
              ...
              final v     = math.max(0.0,
                  math.min(1.0, (_listCtrl.value - delay) / (1.0 - delay)));
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: result.color.withOpacity(0.2), width: 1),
          boxShadow: [BoxShadow(color: result.color.withOpacity(0.06),
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        child: Center(child: Text(result.icon,
                            ...
                            Row(children: [
                              ...
                              if (result.exams.length > 1) ...[
                                ...
                                Container(
                                  ...
                                  child: Text('+${result.exams.length - 1} more',
                      ...
                      Column(
                        ...
                        children: [
                          RichText(
                            text: TextSpan(children: [
                              TextSpan(text: '${result.marks}',
                                  style: TextStyle(color: result.gradeColor,
                                      fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: -1)),
                              TextSpan(text: '/${result.totalMarks}',
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: result.gradeColor.withOpacity(0.35)),
                            ),
        ...
        children: [
          Row(children: [
            ...
            Icon(color == const Color(0xFF00D4AA)
                ? Icons.emoji_events_outlined : Icons.flag_outlined,
                color: color, size: 16),
          ]),
      ...
      builder: (ctx, scrollCtrl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            Row(
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (result.className.isNotEmpty)
                        Text(result.className, style: TextStyle(
                ...
                Container(
                  ...
                  child: Text(result.grade, style: TextStyle(color: result.gradeColor,
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Latest Marks', style: TextStyle(
                        ...
                        const SizedBox(height: 6),
                        Text(result.gradeLabel, style: TextStyle(color: result.gradeColor,
            ...
            if (result.exams.length > 1) ...[
              const SizedBox(height: 20),
              Text('ALL EXAMS', style: TextStyle(color: Colors.white.withOpacity(0.35),
                  fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1.5)),
              const SizedBox(height: 12),
              ...result.exams.map((exam) {
                final m   = exam['marks_obtained'] as int?  ?? 0;
                final pct = exam['percentage']     as double? ?? 0.0;
                final g   = exam['grade']          as String? ?? '';
                final et  = exam['exam_type']      as String? ?? '';
                final ed  = exam['exam_date']      as String? ?? '';
                Color gc = pct >= 80 ? const Color(0xFF00D4AA) :
                pct >= 60 ? const Color(0xFFFFB347) : const Color(0xFFFF6584);
                return Container(
                  ...
                  child: Row(children: [
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(et, style: const TextStyle(color: Colors.white,
                          fontSize: 13, fontWeight: FontWeight.w700)),
                      if (ed.isNotEmpty)
                        Text(ed, style: TextStyle(color: Colors.white.withOpacity(0.35), fontSize: 11)),
                    ])),
                    Text('$m/${result.totalMarks}',
                        style: TextStyle(color: gc, fontSize: 16, fontWeight: FontWeight.w800)),
                    ...
                    Container(
                      ...
                      child: Text(g, style: TextStyle(color: gc,
